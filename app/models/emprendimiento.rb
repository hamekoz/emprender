...
#                        :observaciones
  ...
  validate :mail, :uniqueness, :unless => :mail.blank?
  validate :mail, :format => { :with => /^[^@][\w.-]+@[\w.-]+[.][a-z]{2,4}$/i }, :unless => :mail.blank? 
  validate :web, :format => { :with => /(^$)|(^(http|https):\/\/[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(([0-9]{1,5})?\/.*)?$)/ix} , :unless => :web.blank? 
